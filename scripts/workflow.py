...
def load_mock_data():
    ...
    if not mock_data_file.exists():
        ...
        print("   Please run: python scripts/convert_mock_data.py")
        sys.exit(1)
...
def step2b_assign_to_exam(exam_id, roles, mock_data, exam_index):
    ...
    print(f"\n🔑 Assigning examiners to Exam {exam_id} "
          f"(Section A → Examiner 1, Section B → Examiner 2)...")
    print(f"   🎭 Anonymity: assignments are PRIVATE on-chain — no examiner "
          f"ever sees who else is assigned.")
    for slot, ex in enumerate(examiners_for_exam[:2]):
        addr        = ex["address"]
        name        = ex.get("name", addr[:10])
        section     = config.SECTIONS["A"] if slot == 0 else config.SECTIONS["B"]
        sec_name    = config.get_section_name(section)
        try:
            tx = rbac_contract.functions.assignExaminerToExam(
                addr, exam_id, section
            ).transact({"from": admin})
            base.web3.eth.wait_for_transaction_receipt(tx)
            print(f"   ✅ {name} assigned as Section {sec_name} examiner "
                  f"for Exam {exam_id}")
        except Exception as e:
...
def step5_submit_marks(exam_id, script_ids, mock_data, roles, exam_index=0):
    print_step("5", "Section-Based Marks Submission (Section A + Section B → 100)")
    ...
    # Must have exactly 2 examiners assigned to this exam (one per section)
    key               = str(exam_index)
    ...
    if len(assigned_examiners) < 2:
        ...
        print(f"   Re-run convert_mock_data.py and assign 2 examiners per exam.")
        return

    ex1 = assigned_examiners[0]   # Section A
    ex2 = assigned_examiners[1]   # Section B
    print(f"\n   Section A Examiner: {ex1['name']} ({ex1['address'][:12]}...)")
    print(f"   Section B Examiner: {ex2['name']} ({ex2['address'][:12]}...)")
    print(f"   🎭 Anonymity: each examiner submits ONLY their own section — "
          f"neither sees the other's marks or identity.")

    # Transition to EVALUATION
    exam_iface.update_exam_state(exam_id, "EVALUATION", admin)

    print(f"\n👨‍🏫 Examiners submitting marks out of 50 each (per section)...\n")

    for i, script_id in enumerate(script_ids):
        sm            = exam_marks['student_marks'][i]
        total_initial = sm['initial_marks']   # 0-100 from mock data

        # Pre-split marks from mock data: examiner1 → Section A, examiner2 → Section B
        ...
        print(f"   📝 {script_id}:")
        print(f"      Section A ({ex1['name'][:30]}): {ex1_marks}/50")
        ...
        if not r1:
            print(f"      ❌ Section A submission failed")
            continue

        print(f"      Section B ({ex2['name'][:30]}): {ex2_marks}/50")
        ...
        if r2:
            ...
            print(f"      ❌ Section B submission failed")
...
def step6_scrutiny(exam_id, script_ids, mock_data, roles, exam_index=0):
    print_step("6", "Scrutiny — Return to Section Examiner → Revise → Recheck & Approve")
    ...
    # Per-section examiners for this exam: [0] = Section A, [1] = Section B
    assigned_examiners = mock_data.get("exam_examiners", {}).get(key, [])
    if len(assigned_examiners) < 2:
        print(f"\n❌ Exam {exam_id} needs exactly 2 assigned examiners for "
              f"section-aware scrutiny.")
        return
    ex_a = assigned_examiners[0]
    ex_b = assigned_examiners[1]
    section_examiner = {"A": ex_a, "B": ex_b}
    ...
    print(f"\n🔍 Scrutinizer reviewing scripts (sees only script IDs + sections)...")
    print(f"   🎭 Anonymity: the scrutinizer NEVER sees who marked a section —")
    print(f"   a returned section goes ONLY to that section's examiner, and")
    print(f"   the OTHER examiner is never notified.\n")

    # Randomly pick 1 or 2 scripts to scrutinize
    ...
    scrutiny_count  = 0
    first_round     = True   # first pick also demos a REJECT round (re-scrutiny)

    for idx, script_id in enumerate(script_ids):
        sm = exam_marks['student_marks'][idx]

        # Current per-section marks (mock data: examiner1 → A, examiner2 → B)
        section_marks = {
            "A": min(sm.get("examiner1_marks", sm['initial_marks'] // 2), 50),
            "B": min(sm.get("examiner2_marks", sm['initial_marks'] - sm['initial_marks'] // 2), 50),
        }

        if idx in scripts_to_change:
            # ── Pick ONE section of this script to scrutinize ─────────────
            section = random.choice(["A", "B"])
            current = section_marks[section]

            # ── Step A: Scrutinizer RETURNS marks + comment to the section examiner ──
            # (marks are NOT changed on-chain at this step)
            change    = random.randint(1, 7)
            suggested = min(current + change, 50)
            comment   = random.choice(scrutiny_reasons)

            print(f"\n   🔁 {script_id} Section {section}: Scrutinizer suggests "
                  f"{suggested}/50 (was {current}/50)")
            print(f"      Comment: {comment}")
            print(f"      📩 Only the Section {section} examiner is notified — "
                  f"the other examiner never sees this.")

            r1 = result_iface.return_script_for_scrutiny(
                script_id       = script_id,
                section         = section,
            ...
            if not r1:
                print(f"      ❌ Return to examiner failed")
                continue

            # ── Step B: Section examiner verifies and revises ─────────────
            revised = suggested if random.random() < 0.7 else current
            note    = ("Accepted scrutinizer suggestion after verification"
                       if revised == suggested else
                       "Verified script — kept original marks")
            print(f"   ✍️  {script_id} Section {section}: "
                  f"{section_examiner[section]['name']} revises → {revised}/50")

            r2 = result_iface.respond_to_scrutiny(
                script_id     = script_id,
                section       = section,
                revised_marks = revised,
                note          = note,
                from_account  = section_examiner[section]['address']
            )
            if not r2:
                print(f"      ❌ Examiner response failed")
                continue

            # ── Step C: Same scrutinizer RECHECKS ──────────────────────────
            if first_round:
                # Demo a rejection: one section goes back for a second round
                first_round = False
                rej_comment = "Recheck: calculation error in revised marks — please re-verify"
                print(f"   ❌ {script_id} Section {section}: scrutinizer "
                      f"REJECTS the revision")
                r_rej = result_iface.reject_scrutiny(
                    script_id, section, rej_comment, scrutinizer
                )
                if not r_rej:
                    print(f"      ❌ Rejection failed")
                    continue
                print(f"      🔁 New round: section back with "
                      f"{section_examiner[section]['name']}...")
                r2b = result_iface.respond_to_scrutiny(
                    script_id     = script_id,
                    section       = section,
                    revised_marks = revised,
                    note          = "Re-verified after scrutinizer's recheck comment",
                    from_account  = section_examiner[section]['address']
                )
                if not r2b:
                    print(f"      ❌ Second response failed")
                    continue

            print(f"   ✅ {script_id} Section {section}: scrutinizer "
                  f"rechecks & approves...")
            r3 = result_iface.approve_scrutiny(script_id, section, scrutinizer)
            if not r3:
            ...
            print(f"      Section {section} APPROVED: {current} → {revised} "
                  f"({'+' if delta >= 0 else ''}{delta})")

            # Update in-memory so reports reflect scrutinized marks
            if section == "A":
                sm['examiner1_marks'] = revised
            else:
                sm['examiner2_marks'] = revised
            sm['final_marks']     = sm.get("examiner1_marks", 0) + sm.get("examiner2_marks", 0)
            sm['scrutiny_change'] = sm.get('scrutiny_change', 0) + delta
            sm['scrutiny_reason'] = comment
        else:
            print(f"   ─  {script_id}: Confirmed unchanged "
                  f"(A={section_marks['A']}/50 + B={section_marks['B']}/50 "
                  f"= {section_marks['A'] + section_marks['B']}/100)")

    print(f"\n✅ Step 6 Complete — {scrutiny_count} section(s) scrutinized & approved")
    ...
    for i, script_id in enumerate(script_ids):
        ...
        try:
            ...
            for j, entry in enumerate(trail, 1):
                ...
                ctype = entry[5]
                # changeTypes end with the section suffix ("EXAMINER_A", ...)
                if ctype[-2:] in ("_A", "_B"):
                    base    = ctype[:-2]
                    section = ctype[-1]
                else:
                ...
                if base == "EXAMINER":
                    print(f"   [{j}] EXAMINER SECTION {section:<2}: {entry[1]}/50")
                    print(f"        By     : {entry[3][:20]}...")
                    print(f"        Reason : {entry[4]}")
                elif base == "SCRUTINY_RETURN":
                    print(f"   [{j}] RETURN   S{section} : {entry[0]} → {entry[1]} (suggested)")
                    print(f"        By     : {entry[3][:20]}...")
                    print(f"        Reason : {entry[4]}")
                elif base == "SCRUTINY_RESPONSE":
                    print(f"   [{j}] REVISE   S{section} : {entry[0]} → {entry[1]}")
                    print(f"        By     : {entry[3][:20]}...")
                    print(f"        Reason : {entry[4]}")
                elif base == "SCRUTINY_APPROVE":
                    print(f"   [{j}] APPROVE  S{section} : recheck approved  ({entry[1]}/50)")
                    print(f"        By     : {entry[3][:20]}...")
                elif base == "SCRUTINY_REJECT":
                    print(f"   [{j}] REJECT   S{section} : {entry[0]} → {entry[1]}")
                    print(f"        By     : {entry[3][:20]}...")
                    print(f"        Reason : {entry[4]}")
                else:
            ...
            print(f"   [1] EXAMINER SECTION A: {ex1}/50")
            print(f"   [2] EXAMINER SECTION B: {ex2}/50  "
